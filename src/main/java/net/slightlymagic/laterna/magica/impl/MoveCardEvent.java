...
/**
 * The class MoveCardEvent. MoveCardEvents are posted to notify a listener for the movement of a {@link CardObject}
 * between two {@link Zone}s. Such events are also posted after creation and before deletion of objects (such as
 * newly created/destroyed tokens, ability objects put on/leaving the stack etc.), but not per se. The event is the
 * result of the object being put into a zone after creation, or removing from the zone before deleting.
 * 
